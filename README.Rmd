...
The documented effects of climate change have the potential to alter the underwater ecosystem in considerable ways.  While more analysis is needed, I hope this project will lead to further analysis of temperature shifts and the development of future models from which STINAPA and others can further their work.
...
I use a log transformation and adjust K values to tune the GAM.  The model summary confirms the results are statistically significant with p-values under 0.001.  The concurvity tests indicate high levels of concurvity between the input variables.  While not optimal for many models, this is common with time smooths that utilized multiple units of measure.  I propose that the benefit of smoothing across annual, weekly, and daily segments outweighs the loss in interpretability as each variable is a unit of time.  

The residual plots do not indicate notable trends, residuals appear to have a Gaussian distribution, and a linear response; the GAM residuals are similar in nature with those of the ARIMA models.  The *vis.gam* function provides a tidy visualization of the interaction between the *week* and *year* variables.
...
As with any predictive model, results can be improved through further research, tuning, and testing.  Adjustments to data collection, outlier thresholds, pre-processing, model selection, and hyper-tuning parameters are all relevant areas for improvement.  It is my hope that these models provide a baseline from which further modeling can be generated.
