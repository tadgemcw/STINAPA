...
ts_complete <- seadec
train <- ts_complete[1:104]
test <- ts_complete[105:length(ts_complete)]
...
diff1 <- diff(ts_complete)
diff2 <- diff(diff1)
par(mfrow = c(2,1))
acf2(diff2)

traindiff <- diff(train, lag=52)
traindiff2 <- diff(train)
par(mfrow = c(2,1))
acf2(traindiff, max.lag = 50)
```
...
trainmod <- sarima(train, p=1,d=1,q=2,P=1,D=1,Q=0,S=27)
trainmod


#best model for complete data set
model <- sarima(ts_complete, p=2,d=1,q=2,P=0,D=1,Q=1,S=6)
model

# Forcasting
# use sarima.for(ts_complete, n.ahead = number of periods to forcase, 1,1,1). Last args match model params.

pred <- sarima.for(train, n.ahead=length(test), p=1,d=1,q=2,P=1,D=1,Q=0,S=27)

#RMSE
sqrt(mean((pred$pred - test)^2))
```
