...
CompleteWkTemp20 <- avgTemp20_weekly[,-3]
ts_complete <- ts(avgTemp20_weekly$full_wk_temp)
```
...
Below, both models are trained on 90% of the time series, then used to predict the remaining 10% of the known values.  When the estimated values are compared with the actual values, the both models appear to fit well.  Additionally, the seasonal model has a marginally smaller root-mean-square-error (RMSE) than the non-seasonal model.
...
# Below, training new model
mlmodel <- train(full_temp ~ date + year + week,
                 data = mltrain, 
                 method = "gam",
                 verbose = FALSE)
...
# Below, predicting test values and visualizing accuracy.
mltest$predictions <- predict(mlmodel, newdata=mltest)


accuracy(mltest$full_temp, predictions)


ggplot(avgTemp20_full, aes(x=date, y=full_temp)) +
  geom_point() +
  geom_line(data=mltest,
            aes(date, predictions), 
            color="blue",
            size = 1)

####################### other GAM model. needs transformations of inputs #####
gam_model <- gam(full_temp ~ s(as.numeric(date))+ year + week, data=mltrain)
summary(gam_model)

mltest$gam_pred <- predict(gam_model, newdata=mltest)


accuracy(mltest$full_temp, mltest$gam_pred)


ggplot(avgTemp20_full, aes(x=date, y=full_temp)) +
  geom_point() +
  geom_line(data=mltest,
            aes(date, mltest$gam_pred), 
            color="blue",
            size = 1)
```
...
```{r tbats model}
avgTemp20_full$full_temp %>% tbats() %>% forecast(h=365) %>% autoplot()
```