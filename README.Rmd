...
temp20mm %>% filter(min_temp < 23) %>% arrange(date)  %>%
  rename("Date" = date, "Site" = site, "Depth (m)" = depth_m) %>%
...
FullSeq <- seq.Date(from = min(avgTemp_12m$date), to = max(avgTemp_12m$date), by = 1)
MissingDate <- FullSeq[!FullSeq %in% avgTemp_12m$date]
MissingDate

# Creating ts analysis
timeseries <- ts(avgTemp_12m$avg_temp)
plot(timeseries)
