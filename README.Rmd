...
avg_daily_temps <- avg_daily_temps %>% 
  mutate(year = year(date),
         ...
         week = week(date),
         day = day(date),
         rolling_wk_avg = rollmean(avg_temp, k = 7, fill = NA, align = "right"))


highs_2022 <- avg_daily_temps %>%
    group_by(date) %>%
    filter(year == "2022")

avg_daily_temps %>% filter(year != "2022") %>%
  ggplot(aes(week, rolling_wk_avg)) + 
  geom_smooth() +
  geom_line(data = highs_2022, aes(x = week, y = avg_temp))
