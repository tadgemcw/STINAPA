...
ts_complete <- ts(avgTemp20_weekly$wk_temp)
```
...
mltest$caret.pred <- predict(mlmodel, newdata=mltest)

accuracy(mltest$full_temp, mltest$caret.pred)
...
gam.mod <- gam(full_temp ~ te(log(as.numeric(date))) + te(log(week), k=52)+ te(log(year), k=3),
               data = mltrain,
               method = "REML") 
summary(gam.mod)
gam.check(gam.mod)
...
plot(gam.mod, scheme=2)

```



```{r mgcv gam predictions}

mltest$gam_preds <- predict(gam.mod, newdata=mltest)

accuracy(mltest$full_temp, mltest$gam_preds)
