...
par(mfrow = c(2,1))
plot(ts_complete)
plot(diff(diff(ts_complete), lag = 365))
...
# Below, ploting auto correlation and partial auto correlation.
# Should it be diffed????
acf2(ts_complete)
acf2(diff(diff(ts_complete)))
...
sarima(ts_complete, 3,0,0) #AR model
#sarima(ts_complete, 0,0,1) #MA model
sarima(ts_complete, 3,0,1) #ARMA model.  Look for lowest AIC and BIC residuals.
sarima(ts_complete, 1,1,1) #ARIMA model is used on base data when the differenced data looks like ARMA of undifferenced data.
sarima(ts_complete, 2,1,1)

#seasonal
#sarima(ts_complete, p,d,q, P,D,Q, S)
sarima(ts_complete, 3,0,1,1,0,0,60)
#sarima.for(ts_complete, n.ahead=12, pdq,PDQ,S)
