...
# create a running avg of weekly highs from 2020-2021, then create a running avg of daily highs for 2022.  Then calc DHW from difference?
...
#Breaking out date parts and adding a 7-day rolling average.
...
avg_daily_temps %>% filter(year(date) != "2022") %>%
  ggplot(aes(x = week(date), y = avg_temp)) + 
  geom_smooth(aes(color = "Hist. Avg.")) +
  geom_line(data = avg_daily_temps_2022, aes(y = avg_temp, color = factor(year(date)))) +
  scale_x_continuous(breaks = seq(2, 52, 2)) +
  scale_color_manual(values = c("firebrick2", "blue3"), name = "Year") +
  xlab("Weeks") +
  ylab("Water Temperature (c)") +
  ggtitle("Comparing 2022 with 2020-2021 Average") +
  theme_bw()
