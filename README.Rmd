...
I also plotted the change between monthly mean temperatures and the recorded maximum temperatures for each  month.  Notably, there appears to be an upward trend from 2020 through August, 2022.  This trend could be further explored in conjunction with work from the National Oceanic and Atmospheric Association (NOAA) regarding coral bleaching events and the calculation of degree heating days.[^2]  

[^2]: Lachs, L.; Bythell, J.C.; East, H.K.; Edwards, A.J.; Mumby, P.J.; Skirving, W.J.; Spady, B.L.; Guest, J.R. Fine-tuning heat stress algorithms to optimise global predictions of mass coral bleaching. Remote Sens. 2021, 13, 2677. https://doi.org/10.3390/rs13142677
...
# Plotting Max Above Mean by Year
sum_all <- temp_mma %>% 
  group_by(year, month) %>%
  summarize(avg.temp = round(mean(avg_temp),2),
            max.temp = round(max(max_temp),2),
            high_above_avg = round(max.temp - avg.temp, 2))
# Full date
sum_all$date <- as.Date(with(sum_all,paste(year, month, "01", sep="-")),"%Y-%m-%d")


ggplot(sum_all, aes(date, high_above_avg, color = factor(year))) +
  geom_col(fill = "white") +
  geom_smooth(aes(x=date, y=high_above_avg, color = year), method = "lm", se = FALSE, color = "orangered") +
  scale_color_manual(values = c("deepskyblue1","darkviolet","darkblue")) +
  labs(x = "Time",
       y = "Temperature (c)",
       color = "Year",
       title = "Maximum Temperature Above Monthly Mean")
...
tab <- with(temp_mma, table(site, depth_m))
as_flextable(tab)
...
I selected a Generalize Additive Model (GAM) as an alternative modeling method.  As GAMs are composed of multiple smooth functions, I hoped the approach would yield an accurate non-linear model derived from separate annual, weekly, and daily functions to determine water temperature.  For this model I use the daily averages of water temperatures instead of the weekly averaged data used in the ARIMA models as the GAM results are more accurate with the added data.  Seasonally decomposed Interpolation is again used to fill the missing training data.
