...
This analysis is based on a series of aggregations and transformations of the data provided.  All calculations and visualizations are performed using R. R packages for this project include knitr, readxl, tidyverse, lubridate, flextable, rempsyc, asta, imputeTS, and forecast. The report is formatted in Markdown, and all versions have been pushed to a github repository.
...
predictions <- predict(mlmodel, mltest)

accuracy(mltest$full_temp, predictions)


ggplot(mltest, aes(x=date, y=full_temp)) +
  geom_point() +
  geom_point(aes(mltest$date, predictions), color = "red")

ROC <- roc(mltest$full_temp, predictions)
plot(ROC, col = "blue")
auc(ROC) 
```
