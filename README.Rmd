...
I find it interesting that the isolated low temperature events in 2022 were all recorded at six meters.  No significant low temperatures were recorded at lower depths.  

```{r review of low outliers, echo=FALSE}

# 2020 low temp outliers
temp20mm %>% filter(temp_c < 22) %>% arrange(date)  %>%
  rename("Date" = date, "Site" = site, "Depth (m)" = depth_m, "Temp (c)" = temp_c) %>%
  nice_table(title = "2020 Temperatures Below 22 Degrees")

#2021 low temp outliers
temp21mm %>% filter(temp_c < 22) %>% arrange(date)  %>%
  rename("Date" = date, "Site" = site, "Depth (m)" = depth_m, "Temp (c)" = temp_c) %>%
  nice_table(title = "2021 Temperatures Below 22 Degrees")

#2022 low temp outliers
temp22mm %>% filter(temp_c < 22) %>% arrange(date)  %>%
  rename("Date" = date, "Site" = site, "Depth (m)" = depth_m, "Temp (c)" = temp_c) %>%
  nice_table(title = "2022 Temperatures Below 22 Degrees")
```

### Degree Changes

...talk about significance of 4 Degree Heating Week (DHW) degree changes in x number of hours/days as indicator of coral bleaching.
